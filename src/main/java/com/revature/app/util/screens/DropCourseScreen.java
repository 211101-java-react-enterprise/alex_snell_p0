...
public class DropCourseScreen extends Screen {

    public DropCourseScreen(StateAction action, AppState state) {
        super("DropCourseScreen", "/dropcourse", action, state);
    }
    ...
    public void render() {
        ...
        try {
            System.out.println("Enter a course ID to drop the course or nothing to return to the Dashboard.");
            int coursesListIndex = Integer.parseInt(this.state.readLine());
            targetCourse = myCourses.get(coursesListIndex);
            this.state.courseService.deleteRegistration(this.state.userService.getSessionUser().getId(), targetCourse.getId());
            System.out.println("Dropped course and returning to dashboard.");
            this.state.router.navigate("/dashboard");
        } catch (Exception e) {
            System.out.println("Returning to dashboard...");
            this.state.router.navigate("/dashboard");
